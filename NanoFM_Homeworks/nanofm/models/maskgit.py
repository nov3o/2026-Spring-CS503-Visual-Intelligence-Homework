...
class MaskGIT(nn.Module):
    ...
    def __init__(
        ...
        self.positional_embedding = nn.Parameter(torch.zeros(1, seq_len, dim))
    ...
    def forward_model(self, x: torch.LongTensor, mask: torch.BoolTensor) -> torch.Tensor:
        ...
        embeds = torch.where(mask.unsqueeze(-1), self.mask_token.expand(B, L, -1), embeds)

        # Add the positional embeddings to the tokens
        embeds = embeds + self.positional_embedding
    ...
    def generate_random_mask(self, seq: torch.Tensor) -> torch.BoolTensor:
        ...
        # Generate and return a random mask of shape (B, L), where
        ...
        num_masked = torch.randint(1, L + 1, (B, 1), device=seq.device)  # [B, 1]
        ranks = torch.argsort(torch.argsort(noise, dim=1), dim=1)  # [B, L], 0-indexed
        return ranks < num_masked  # True where rank < num_masked → masked
    ...
    def compute_ce_loss(self, logits: torch.Tensor, target_seq: torch.LongTensor, ignore_index: int = -100) -> torch.Tensor:
        ...
        return F.cross_entropy(logits.reshape(-1, logits.size(-1)), target_seq.reshape(-1), ignore_index=ignore_index)
    ...
    def generate(
        ...
        for step, k in enumerate(schedule):
            ...
            # Get the indices of masked tokens. Shape: [M,] (M = number of masked tokens)
            ...
            top_k_idx = confidence.topk(k).indices
            selected_positions = masked_indices[top_k_idx]
