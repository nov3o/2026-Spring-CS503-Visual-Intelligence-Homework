...
class GPT(nn.Module):
    ...
    def forward_model(self, x: torch.LongTensor) -> torch.Tensor:
        ...
        causal_mask = torch.ones(L, L, device=x.device, dtype=torch.bool).tril().unsqueeze(0)
    ...
    def compute_ce_loss(self, logits: torch.Tensor, target_seq: torch.LongTensor, padding_idx: int = -100) -> torch.Tensor:
        ...
        return F.cross_entropy(logits.reshape(-1, logits.size(-1)), target_seq.reshape(-1), ignore_index=padding_idx)
    ...
    def generate(
        ...
        for _ in range(self.max_seq_len - len(context)):
            ...
            samples, _ = sample_tokens(logits[:, -1, :], temperature=temp, top_k=top_k, top_p=top_p)
            ...
            if eos_idx is not None and (samples == eos_idx).all():
                break
