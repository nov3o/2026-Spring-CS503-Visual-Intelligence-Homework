...
class FourM(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Define the input embedding layer using self.vocab_size
        ...
        # Define the encoder -> decoder projection. This is simply a linear layer mapping dim -> dim.
        self.dec_context_proj = nn.Linear(dim, dim)

        # Initialize decoder output projection
        # Define the output projection layer using self.vocab_size
        self.to_logits = nn.Linear(dim, self.vocab_size, bias=False)

        # Initialize norm layers
        # Define the encoder output layer normalization
        self.enc_norm = LayerNorm(dim)
        # Define the decoder output layer normalization
    ...
    def forward_encoder(
        ...
        # Embed the input tokens `enc_input_tokens` using the input embedding layer `enc_tok_emb`. Shape: [B, N, D]
        x = self.enc_tok_emb(enc_input_tokens)

        # Embed the input modality IDs `enc_input_modalities` using the input embedding layer `enc_mod_emb`. Shape: [B, N, D]
    ...
    def forward_decoder(
        ...
        # Embed the target modality IDs `dec_input_modalities` using the embedding layer `dec_mod_emb`. Shape: [B, M, D]
    ...
    def forward_model(
        ...
        # Pass `dec_x` through linear output head `to_logits` to compute the logits. Shape: [B, M, vocab_size]
    ...
    def generate_one_modality_roar(
        ...
        chunks = torch.split(shuffled_positions, schedule)
        ...
        dec_input_positions_list = [c.unsqueeze(0) for c in chunks]
        ...
        for step, k in enumerate(schedule):
            ...
            # Sample new tokens for the predicted_logits
            ...
            # Concatenate the new tokens to the encoder input tokens for the next step
