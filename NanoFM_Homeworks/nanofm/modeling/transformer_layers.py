...
class Attention(nn.Module):
    ...
    def __init__(self, dim: int, head_dim: int = 64, qkv_bias: bool = False, proj_bias: bool = False):
        ...
        # Using a single fused QKV projection for efficiency
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        q, k, v = rearrange(self.qkv(x), 'b l (three h d) -> three b h l d', three=3, h=self.num_heads).unbind(0)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        k, v = rearrange(self.kv_proj(context), 'b m (two h d) -> two b h m d', two=2, h=self.num_heads).unbind(0)
...
class TransformerTrunk(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        # Forward pass through the transformer trunk
