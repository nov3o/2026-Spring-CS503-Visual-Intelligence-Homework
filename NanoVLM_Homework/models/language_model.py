...
class LanguageModelBlock(nn.Module):
    ...
    def forward_kv(self, x, cos, sin, past_key_value=None):
        ...
        # return both hidden states and the present_key_value
...
class LanguageModel(nn.Module):
    ...
    def forward_kv(self, x, past_key_values=None, attention_mask=None):
        ...
        # past length is 0 if past_key_values is None, otherwise get the length from past_key_values
        past_length = 0 if past_key_values is None else past_key_values[0][0].size(2)

        # position ids start from past length till past length + T
        ...
        for i, block in enumerate(self.blocks):
            # fetch this layer's cache entry (or None on first call)
            past_kv = None if past_key_values is None else past_key_values[i]
            # call block.forward_kv while passing the past_kv alongside with default arguments
        ...
        # return hidden states and the updated kv cached list
